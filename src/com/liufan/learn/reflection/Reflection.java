...
import com.liufan.learn.reflection.dynamicproxy.Hello;
...
public class Reflection {
    ...
    public static void practiceClass() {
        ...
        Class cls1 = String.class; // 方法一：通过一个class的静态变量class获取（当我们获取到某个Class对象时，实际上就获取到了一个类的类型）
        Class cls2;
        try {
    ...
    public static void getExtends() {
        ...
        // 获取interface，如果一个类没有实现任何interface，那么getInterfaces()返回空数组。
        Class[] is = i.getInterfaces(); // 只返回当前类直接实现的接口类型，并不包括其父类实现的接口类型
        for (Class cls : is) {
            System.out.println(cls);
        }
        System.out.println("——————————");
        Class[] ns = n.getInterfaces(); // 父类实现的接口类型
        for (Class cls : ns) {
            System.out.println(cls);
        }
        // 此外，对所有interface的Class调用getSuperclass()返回的是null，获取接口的父接口要用getInterfaces()
        System.out.println(java.io.Closeable.class.getSuperclass()); // null，对接口调用getSuperclass()总是返回null
        Class[] cs = java.io.Closeable.class.getInterfaces();        // 获取接口的父接口要用getInterfaces()
        for (Class cls : cs) {
            System.out.println(cls);
        }
    ...
    public static void dynamicProxy() {
        /*
        所有interface类型的变量总是通过某个实例向上转型并赋值给接口类型变量的，例如：
